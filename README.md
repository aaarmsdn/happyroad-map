...
`config/sgg.json`의 시군구 경계로 각 아파트 좌표의 5자리 `LAWD_CD`를 계산합니다. 국토부 거래와 단지 좌표의 지역 코드가 같고 정규화한 이름이 하나로 일치할 때 갱신합니다. 이름이 다르면 같은 시군구 안에서 주상복합·고층 같은 부가 표기, 차·단지 표기와 SK·LG 한글 표기를 정리한 이름이 서로 포함되고 후보가 하나뿐인 경우만 자동 연결합니다. 검증된 별칭은 `config/price-name-aliases.json`에 명시하며, 숫자가 충돌하거나 후보가 둘 이상이면 잘못 연결하지 않고 건너뜁니다. 네이버는 현재 매물을 확인하는 외부 링크만 제공하며, 호가·매물 스냅샷을 저장하거나 비공식 API를 호출하지 않습니다.
...
- `npm run prices:refresh`: 워크플로 기준 최근 12개월 국토부 실거래를 단지명으로 보수적으로 매칭
