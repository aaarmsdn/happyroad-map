...
import { addApartmentMarkers, addStopMarkers, spreadMarkerPoints, stopTimeColor } from "../public/map-view.js";

test("stop time colors use travel-time bands", () => {
  assert.deepEqual([29, 30, 60, 90, 120].map(stopTimeColor), ["#18864b", "#2774ae", "#d6a01d", "#f07835", "#d83a3a"]);
});
...
test("stop cluster color uses average representative travel time", () => {
  const icons = [];
  const L = {
  ...
  const map = {
    getBounds: () => ({ pad: () => ({ contains: () => true }) }), getZoom: () => 13, setView() {},
    latLngToLayerPoint: ([lat, lng]) => ({ x: lng, y: lat }), layerPointToLatLng: ({ x, y }) => [y, x]
  };
  const stop = (index, minutes) => ({ name: `정류장${index}`, lat: 37.5, lng: 127, entries: [{ routeName: "노선", routeCategory: "출근", minutesToCompany: minutes, companyTime: "08:00", turnName: "통상 출근" }] });
  addStopMarkers({ L, map, layer: {}, groupedStops: new Map([["1", stop(1, 30)], ["2", stop(2, 90)]]), onSelect: () => {} });
  assert.match(icons[0], /--route-color:#d6a01d/);
});
...
test("apartment clusters preserve the exact mean direction opacity", () => {
  const icons = [];
  const L = {
    divIcon: options => { icons.push(options.html); return options; },
    marker: latLng => ({
  ...
  const complexes = new Map([
    ["1", { id: "1", name: "단지1", lat: 37.5, lng: 127 }],
    ["2", { id: "2", name: "단지2", lat: 37.501, lng: 127.001 }]
  ]);
  addApartmentMarkers({
    L, map, layer: {}, category: "전체", complexById: complexes,
    visibleLinks: new Map([
      ["1", { accessDirections: ["출근"] }],
      ["2", { accessDirections: ["출근", "퇴근"] }]
    ]),
    priceOf: () => null, colorOf: () => "#f04438", onSelect: () => {}
  });
  assert.match(icons[0], /opacity:0\.725(?:[;\"])/);
});
