...
test("worker returns a compact road address for a map coordinate", async () => {
  ...
  const response = await handleRequest(new Request("https://worker.test/address?lat=37.5446&lng=127.056", { headers: { origin: env.ALLOWED_ORIGIN } }), env, async url => {
    ...
    if (String(url).includes("coord2regioncode")) return Response.json({ documents: [{ region_type: "B", code: "1120011400" }] });
  ...
  assert.match(upstreams[0], /\/v2\/local\/geo\/coord2address\.json/);
  assert.match(upstreams[1], /\/v2\/local\/geo\/coord2regioncode\.json/);
  assert.deepEqual(await response.json(), { address: "서울 성동구 성수일로 12-3", parcelIdentity: "성수동1가|676-5", regionCode: "11200" });
});
